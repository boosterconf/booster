...
## Getting started
    ...
    Check out the code: git clone git@github.com:rootsconf/rootsconf2011.git
    $ gem update --system
    $ gem install bundler
...
Setup:
    ...
    $ heroku keys:add
    $ cd rootsconf2011
    $ git remote add production git@heroku.com:rootsconf.git
    $ git remote add staging git@heroku.com:stagingroots.git
...
Fool around:
    ...
    $ heroku db:pull --app [stagingroots|rootsconf]

Update (push):

    $ git push [production|staging|master]
    #DB changes? remember to migrate the server
    $ heroku rake db:migrate --app [stagingroots|rootsconf]
...
Heroku SendGrid:
    ...
    $ heroku config --long --app rootsconf
...
Otherwise; fork the project (http://github.com:rootsconf/rootsconf - click the "fork project" button).

For working on your fork (replace <username> with 'rootsconf' if you are allowed to work on the main project).

    $ git clone git@github.com:<username>/rootsconf.git
    $ cd rootsconf
    ...
    git push (Push local changes to the central (origin) repository to github.com/<username>/rootsconf)
